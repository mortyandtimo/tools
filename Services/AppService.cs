...
namespace IntelliCoreToolbox.Services
{
    ...
    public class AppService : INotifyPropertyChanged
    {
        ...
        private async Task InitializeData()
        {
            ...
            System.Diagnostics.Debug.WriteLine($"🎯 已添加 {favoriteAppsData.Length} 个默认收藏应用，当前FavoriteApps数量: {FavoriteApps.Count}");

            CreateLoopingFavorites();
            System.Diagnostics.Debug.WriteLine($"🎯 已创建循环收藏列表，LoopingFavoriteApps数量: {LoopingFavoriteApps.Count}");

            // 初始化全部应用 (从AppCenterViewModel迁移)
            var allAppsData = new[]
            {
            ...
            System.Diagnostics.Debug.WriteLine($"🎯 已添加 {allAppsData.Length} 个默认全部应用，当前AllApps数量: {AllApps.Count}");

            // 初始化合集 (从AppCenterViewModel迁移)
            InitializeCollections();
            System.Diagnostics.Debug.WriteLine($"🎯 已初始化合集，Collections数量: {Collections.Count}");

            // 🎯 加载用户添加的应用
            System.Diagnostics.Debug.WriteLine("🎯 准备加载用户添加的应用...");
            await LoadUserApps();
        ...
        private async Task LoadUserApps()
        {
            try
            {
                Console.WriteLine($"🎯 LoadUserApps开始: 文件路径={_userAppsFilePath}");
                Console.WriteLine($"🎯 文件存在检查: {File.Exists(_userAppsFilePath)}");
                
                if (!File.Exists(_userAppsFilePath))
                {
                    Console.WriteLine("用户应用配置文件不存在，跳过加载");
                    return;
                }

                var jsonString = File.ReadAllText(_userAppsFilePath);
                Console.WriteLine($"🎯 读取到JSON内容长度: {jsonString.Length}");
                ...
                Console.WriteLine($"🎯 反序列化结果: config={config != null}, UserApps={config?.UserApps?.Count ?? 0}");

                if (config?.UserApps != null)
                {
                    _userAddedApps.Clear();
                    _userAddedApps.AddRange(config.UserApps);
                    Console.WriteLine($"🎯 已清理并添加{config.UserApps.Count}个用户应用到_userAddedApps");
                    ...
                    foreach (var app in config.UserApps)
                    {
                        try
                        {
                            ...
                            {
                                Console.WriteLine($"⚠️ 文件名提取失败，使用原始名称: {pathEx.Message}");
                                displayName = app.Name;
                            }
                            ...
                            Console.WriteLine($"🎯 用户应用已添加到AllApps: Name={displayName}, Path={app.Path}");
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine($"❌ 处理用户应用失败: Name={app.Name}, Path={app.Path}, Error={ex.Message}");
                            // 继续处理下一个应用，不中断整个过程
                        }
                    }

                    Console.WriteLine($"🎯 LoadUserApps完成: 总共添加{addedToAllAppsCount}个用户应用到AllApps");
                    Console.WriteLine($"🎯 当前AllApps总数: {AllApps.Count}");
                    Console.WriteLine($"🎯 当前FavoriteApps总数: {FavoriteApps.Count}");
